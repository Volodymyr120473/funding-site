...
exports.getFundingScreenerHandler = getFundingScreenerHandler;
exports.getNegativeFundingHandler = getNegativeFundingHandler;
...
function directionFromQuery(q) {
    const v = String(q ?? "").toLowerCase().trim();
    return v === "positive" ? "positive" : "negative";
}
function exchangeFromQuery(q) {
    const v = String(q ?? "").toLowerCase().trim();
    return v === "binance" ? "binance" : "bybit";
}
function defaultsByDirection(direction) {
    const fundingCut = direction === "positive"
        ? envNum("FUNDING_CUT_POS", 0.00005)
        : envNum("FUNDING_CUT_NEG", -0.0001);
    const alertFundingCut = direction === "positive"
 ...
 */
async function getFundingScreenerHandler(req, res) {
    try {
        ...
        const data = await (0, funding_service_1.getFundingScreener)(filters);
        res.json(data);
    }
    catch (e) {
 ...
 */
async function getNegativeFundingHandler(req, res) {
    try {
